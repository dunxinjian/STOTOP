...
> - **未做/留后**：4F 剩余(强制选租户模态 + 组织树聚合层 + 待办邀请管理页 + 移动端专属租户选择 UI + switch-tenant 返回 context 直接套用免重选)；企微 provider 实装；钉钉字段拆列 + 登录按 `IDP用户身份` 主查；`TenantResolver` 多客户改造(读 X-Tenant-Context+成员关系);PLT/IDP/分发日志表的 **live dev-DB 迁移未跑**（避开并发会话共享库；机制同阶段2 新 EF 表已验、4A/4E 无既有数据形态风险)；**单客户下多租户 UI 全休眠、未真数据端到端验**。
