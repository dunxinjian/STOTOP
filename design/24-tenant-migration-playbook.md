...
> - **3C**（`FinanceSeeder V18` schema + `BasicDataSeeder V1` 建桥；`V19` 终审修）：**方案A（最小改动、P&L 叶数中性）**。`FinOperatingUnit` 加 `F来源类型`/`F来源业务单元ID`(→`FIN辅助核算项目.FID`) **单向交叉引用桥（OU→aux）**：按 (租户, 公司名去"子"→规范名) 匹配 `business_unit` aux（名不一致"城区子公司"vs"城区公司"故禁纯名/纯码 join）——4 网点公司 aux 建桥，**出港业务(方向)/太仓美申(区域自身) 不桥**（避免区域重复计数）。阿米巴报表 `AmoebaPLService`：`business_unit` aux 仍作分组键（`MapToUnit`/映射规则/凭证**零改**→叶数逐行不变），新增 `BuildUnitRegionParentMapAsync` 经闭包最近区域公司(`FKind=Region`)填 `AmoebaUnitData.ParentId`（区域上卷，**纯附加**）。
>   - **~~反标~~已撤（终审 finding·`V19`）**：原反标被桥 aux `F来源类型='FIN经营单元'` 会触发 `AuxiliaryService.UpdateItemByAccountSetAsync` 的"外部来源不可改名"守卫，**静默冻结**这 4 个经营单元 aux 的改名（且 OU 名随公司同步→漂移）。已删反标、桥改 OU 单向；`V19` 把 existing-DB 已反标的 aux `F来源类型/F来源ID` 清 `NULL`。
...
>   - dev 实测：凭证引用 business_unit=0 行、KSF 全 0 行、CfPluginRule pin business_unit=0 行——迁移唯一活跃面 = 5 条 `FinAmoebaMappingRule` + 报表；故 KSF/凭证/Points **零改**（code path 保留，桥兼容）。
> - **3C part-2（区域 scope 取数·最小过滤档·已实施 dev 验证 + 终审修）**：`AmoebaReportScope` 加 `Regions`（区域公司组织节点ID）。`AmoebaPLService.ExpandRegionScopeAsync` 把 `Regions` 展开成其网点公司经营单元 `business_unit` aux 集，**折叠进 `Units` 维**（区域与 Units 同属 business_unit 维→union；区域无单元→哨兵 `-1L` 出空报表非全口径），仅**多期主报表** `GetMultiPeriodReportAsync` 生效（单期不动）。营收/成本/发件(send)公共费按区域正确；**派件(deliver)公共费不按区域缩**（`AggregateDeliverVolume` 仅认 `Outlets`，`outlet↔网点公司` 主数据未接线所限=既有 Units scope 同限），已在响应 `UnmatchedWarnings` 显式声明。
>   - **终审 finding 修（3 条）**：① **billing 静默丢弃(high)**——区域折叠进 `Units`，但 `ApplyScopeFilter` 的 `business_unit` 只认 `dp.BusinessUnitId`，而生产 billing/折旧点无此字段(只 voucher 有)→区域子报表把**全部出港计费营收**剔除。修：多期在 `isSubReport` 时对每点跑 `MapToUnit` 派生 `MappedUnitId` + `AuxiliaryMatches` 的 `business_unit` 加 `MappedUnitId` 回落。② **org-aware 展开(medium)**——展开改为按 (`FOrgId==orgId`, 规范名) 就近解析本 org 的 aux（不再用 org-agnostic 的 `FSourceLegacyAuxId` 桥），修同租户多区域公司同名 aux 折叠错 org。③ 基线打标门控用 `isSubReport`（非 `Units`）避免公共费分母随子报表维漂移。dev 验证：区域192→aux{2,3,4,5}。**仍留**：完整 deliver 件量档（经 SiteCode 桥）/ 连主数据档（`ExpNetworkPoint.FCompanyId` 接线）/ 前端区域选择器（多期报表现未传 Scope）。
...
> **阶段4 as-built（分支 `feat/tenant-isolation-stage4` off `a67271f` · 后端 4A-4E + 整体终审修 + 前端 4F 核心 · System.Tests 63✓ · 前端 type-check✓）**
> - **4A 平台层地基**：`PltTenant/PltPlan/PltSubscription`（System.Entities，**平台级不实现 `ITenantScoped`**——在租户硬墙之上定义租户本身，挂墙则"查租户须先在租户内"死锁；无 F组织ID 故漏标门禁天然放过，表由 `CreateMissingTables` 建）。`SysUser.FIsPlatformAdmin`（列，接入留 4B）。`TenantResolver` 从"恰好=组织树根"解耦为**读唯一非停用 `PLT租户`.FID**（回退组织树根）。`SystemSeeder V13`：加 `F是否平台超管`+回填 admin；`IDENTITY_INSERT PLT租户 FID=根组织id`（保全表 `F租户ID`=PLT租户.FID 不变，行为等价）。
> - **4B 平台层运行时**：`PlatformOnlyAttribute`（动作过滤器：校验 `FIsPlatformAdmin`→403 + 进 `IPlatformScopeFactory` 平台作用域[物理脱离硬墙] + Enter 审计=PlatformAuditMiddleware 意图）挂 `PlatformController`(/api/platform：租户/套餐/订阅 + /idp corp)。`TenantFreezeMiddleware`(D7：`PLT租户.FStatus=欠费冻结` 拒业务写+批量导出→402、放行只读；**单客户正式态=休眠**；15s TTL 缓存 fail-open)。`/api/platform` 纳两中间件 SkipPaths。
> - **4C M9 后端**：`OrgContextMiddleware` 读**可选 `X-Tenant-Context`**（加性·向后兼容，头缺失回退 `TenantResolver`=零破坏；头在则校验 `SysTenantMember` 已接受成员→403）。`ValidateTenantMembershipAsync`（严格成员，admin/平台超管**不旁路**，跨租户走 /api/platform）+ `SwitchTenantAsync`(R6，按 `SYS组织架构.F租户ID` 筛本租户可切换组织+复用 `SwitchOrganizationAsync` 重算)。**头改名（X-Org→X-Tenant 硬切）未做——设计上 X-Org(R8 组织范围) 与 X-Tenant(R9 租户墙) 语义不同，取加性并存**。
> - **4D M8 IdP**：4 表（`IdpExternalCorp`/`IdpUserIdentity` 平台级；`IdpTenantCorpMap`/`IdpDeptMap` `ITenantScoped`；`IdpDeptMap` 有 FOrgId 是映射目标列**非** `IOrgScoped`）。`IdpService`(EnsureExternalCorp/UpsertUserIdentity/ResolveUserByExternal/LinkCorpToTenant + **ResolveLoginTenant 免登多租户 428 消歧** + 邀请 Invite/Accept/Reject/GetPending)。`TenantMemberController`(邀请端点)。`DingTalkAuthController.Login` 加性接线（best-effort 登记身份 + 428 附 token）。**决策：企微=声明桩**(enum+数据模型支持，运行时未实装，需真 corp)；**钉钉字段加性迁移不拆列**(SysUser 钉钉字段照常，`IDP用户身份` 为前向规范存储)；**单客户保钉钉同步自动接受**(邀请强制只对未来跨租户加入)。
> - **4E R4 待办硬化**：`CfTodoDispatchLog`(CardFlow，`ITenantScoped`) + `TodoDispatchLogService`。分发时记 `taskId→(租户,待办)` **权威绑定**；回调据 taskId **精确定位**待办(替代 `FExternalTodoId` 尾缀模糊匹配)+ **幂等去重(暂存标记，调用方一次原子提交)**——修 `DingTalkChannel.HandleTodoStatusChangeAsync` 跨租户回调重放缺口。**未做(留后)**：corp→租户 `IDP企业租户映射` 校验(需 Core 跨模块契约)、`PLT租户.FDefaultTodoChannel` 渠道路由(钉钉/企微/双推)、AccessToken 缓存按租户+corp、SignalR 组租户限定。
> - **4F M9 前端核心**：`request.ts` 注入 `X-Tenant-Context`(加性·与 X-Org-Context 并存,头缺失后端回退单客户根租户,零破坏) + HTTP 428 提示分支。`stores/tenantContext.ts`(currentTenantId/tenants/hasMultipleTenants + init/fetchTenants/doSwitchTenant/clear)。`TenantSwitcher.vue`(镜像 OrgSwitcher,**仅多租户显示**=单客户休眠;切换=清组织上下文+硬重载以新头重引导)挂 AppSidebar 顶部。types + api(my-tenants/switch-tenant/邀请)。router 惰性 init+非阻塞 fetchTenants;登出清租户上下文。移动端共用 request.ts 故头注入天然覆盖。type-check 0 错、改动无新增裸 hex。
> - **整体终审修（对抗式 5维×核实，11 agent，抓 3 确诊+1 潜在，驳回 2）**：① 回调幂等标记非原子先落库→改暂存+调用方一次原子提交；② 冻结整前缀豁免放行成员写→收窄具体只读路径；③ 免登 428 无 token 死锁→428 先发 token；④ 接受邀请多客户跨租户派生写污染 ChangeTracker→catch 剔除挂起 SysScopeGrant(proper 多客户重算待 TenantResolver 改造)。
> - **live dev-DB 迁移已验证(2026-07-04)**：从 worktree 起后端(端口 9010)对共享 dev 库跑 MigrateAll——`=== 数据库迁移完成 ===` + `[System] V13 完成`(IDENTITY_INSERT PLT租户 + admin 回填无错、录 SYS迁移历史)+ `CreateMissingTables 成功 8 张`(PLT×3/IDP×4/CF待办分发日志,schema+唯一索引全对)+ SYS用户 加 F是否平台超管。加性/幂等对并发后端无害。(注：迁移完成【后】撞到无关既有坑——BaselineReferenceDataSeeder 冷启动崩于 FIN科目模板_明细 重复键 (3,1001)，非阶段4，已另派排查。)
> - **未做/留后**：4F 剩余(强制选租户模态 + 组织树聚合层 + 待办邀请管理页 + 移动端专属租户选择 UI + switch-tenant 返回 context 直接套用免重选)；企微 provider 实装；钉钉字段拆列 + 登录按 `IDP用户身份` 主查；`TenantResolver` 多客户改造(读 X-Tenant-Context+成员关系)；**单客户下多租户 UI 全休眠、未真数据端到端验**。
