...
> - **3C part-2（区域 scope 取数·最小过滤档·已实施 dev 验证）**：`AmoebaReportScope` 加 `Regions`（区域公司组织节点ID）。`AmoebaPLService.ExpandRegionScopeAsync` 经 3C 桥反向（区域→SYS组织闭包→网点公司→`FinOperatingUnit.FSourceLegacyAuxId`）把 `Regions` 展开成其网点公司经营单元 `business_unit` aux 集，**折叠进 `Units` 维**（区域与 Units 同属 business_unit 维→union；区域无单元→哨兵 `-1L` 出空报表非全口径），仅**多期主报表** `GetMultiPeriodReportAsync` 生效（单期不动）。营收/成本/发件(send)公共费按区域正确（`IsSubReport`→全口径基线自动生效，分母=全公司）；**派件(deliver)公共费不按区域缩**（`AggregateDeliverVolume` 仅认 `Outlets`，`outlet↔网点公司` 主数据未接线所限=既有 Units scope 同限），已在响应 `UnmatchedWarnings` 显式声明。dev 验证：区域192→aux{2,3,4,5}。**仍留**：完整 deliver 件量档（经 SiteCode 桥）/ 连主数据档（`ExpNetworkPoint.FCompanyId` 接线）/ 前端区域选择器（多期报表现未传 Scope）。
