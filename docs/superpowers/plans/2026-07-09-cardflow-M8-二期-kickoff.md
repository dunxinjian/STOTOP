...
| **M8-D 定义级开关** | ①审批人去重 ②允许发起人撤回(定义级) ③停用节点 skip | F4 发布设置里三个"二期"灰行转真开关 | 三者引擎均无消费（撤回仅批次级 `RevokeBatchAsync`；停用无 disabled-skip）——需引擎新增 |
...
**建议顺序**：A → B → C → D → E 各自独立可发布；F/G 收尾。也可按业务优先级重排。
...
> 继续 CardFlow 设计器二期(M8)。先读 `docs/superpowers/plans/2026-07-09-cardflow-M8-二期-kickoff.md` 建立上下文，再按其拆批表做 **M8-A 发起节点三件**：先用子代理核实引擎对发起范围/代提交/重提重路由的消费现状，判定可真落地 vs 必须占位(守"不做假配置")，锁模块出细化 TDD plan 后小步实现。后端 `build-filter cardflow`/`test-dotnet cardflow`，不新增 cc FType，提交经 hook 门禁，不 push 等我点头。
