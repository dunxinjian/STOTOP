...
{
    private const string Module = "Dormitory";
    ...
    {
        "DOR设施登记", "DOR报修工单", "DOR访客登记", "DOR宿舍楼栋", "DOR房间",
        "DOR入住记录", "DOR床位", "DOR费用记录", "DOR卫生检查",
    };
    ...
    public static void Migrate(STOTOPDbContext ctx)
    {
        ...
        {
            new(1, "阶段0多租户: Dormitory 9张租户表加 F租户ID 隔离键列(NOT NULL DEFAULT 0,不启用过滤器)+租户索引 (2026-07-01)", MigrateV1),
            new(2, "阶段0多租户: Dormitory 存量行 F租户ID 回填到根组织单租户(=根组织id) (2026-07-01)", MigrateV2),
        });
    }
