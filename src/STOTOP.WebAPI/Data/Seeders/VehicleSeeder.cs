...
{
    private const string Module = "Vehicle";
    ...
    {
        "VEH车辆台账", "VEH租赁收费记录", "VEH租赁费用标准",
        "VEH维修记录", "VEH保险记录", "VEH车辆分配",
    };
    ...
    public static void Migrate(STOTOPDbContext ctx)
    {
        ...
        {
            new(1, "阶段0多租户: Vehicle 6张租户表加 F租户ID 隔离键列(NOT NULL DEFAULT 0,不启用过滤器)+租户索引 (2026-07-01)", MigrateV1),
            new(2, "阶段0多租户: Vehicle 存量行 F租户ID 回填到根组织单租户(=根组织id) (2026-07-01)", MigrateV2),
        });
    }
