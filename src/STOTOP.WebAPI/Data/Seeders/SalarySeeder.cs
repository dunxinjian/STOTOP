...
public static class SalarySeeder
{
    ...
    public static void Migrate(STOTOPDbContext ctx)
    {
        MigrationRunner.RunMigrations(ctx, Module, new List<MigrationStep>
        {
            ...
            new(2, "阶段0多租户: Salary 8张租户表加 F租户ID 隔离键列(NOT NULL DEFAULT 0,不启用过滤器)+租户索引 (2026-07-01)", MigrateV2),
            new(3, "阶段0多租户: Salary 存量行 F租户ID 回填到根组织单租户(=根组织id) (2026-07-01)", MigrateV3),
        });
    }
    ...
    {
        "SAL员工薪酬档案", "SAL月度工资单", "SAL工资明细", "SALB分兑换记录",
        "SAL晋升评审", "SAL薪酬档位", "SAL晋升历史", "SAL晋升规则",
    };
