...
public static class KsfSeeder
{
    ...
    public static void Migrate(STOTOPDbContext ctx)
    {
        MigrationRunner.RunMigrations(ctx, Module, new List<MigrationStep>
        {
            ...
            new(2, "阶段0多租户: KSF 6张租户表加 F租户ID 隔离键列(NOT NULL DEFAULT 0,不启用过滤器)+租户索引 (2026-07-01)", MigrateV2),
            new(3, "阶段0多租户: KSF 存量行 F租户ID 回填到根组织单租户(=根组织id) (2026-07-01)", MigrateV3),
        });
    }
    ...
    {
        "KSF结果明细", "KSF结果", "KSF岗位方案明细", "KSF岗位方案", "KSF指标定义", "KSF员工经营单元映射",
    };
