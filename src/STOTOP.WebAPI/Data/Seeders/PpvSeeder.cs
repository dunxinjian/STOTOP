...
public static class PpvSeeder
{
    ...
    public static void Migrate(STOTOPDbContext ctx)
    {
        MigrationRunner.RunMigrations(ctx, Module, new List<MigrationStep>
        {
            ...
            new(2, "阶段0多租户: PPV 4张租户表加 F租户ID 隔离键列(NOT NULL DEFAULT 0,不启用过滤器)+租户索引 (2026-07-01)", MigrateV2),
            new(3, "阶段0多租户: PPV 存量行 F租户ID 回填到根组织单租户(=根组织id) (2026-07-01)", MigrateV3),
        });
    }

    /// <summary>阶段0 多租户隔离：需加 F租户ID 的 4 张租户表（全覆盖，见 design/24-tenant-migration-playbook.md）。</summary>
    private static readonly string[] Phase0TenantTables =
    {
        "PPV违规记录", "PPV产值记录", "PPV产值模板", "PPV月度汇总",
    };
