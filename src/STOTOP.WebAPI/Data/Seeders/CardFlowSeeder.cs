...
public static class CardFlowSeeder
{
    ...
    /// <summary>阶段1收尾(用户裁定编号序列按租户隔离)：CF编号序号 加 F租户ID 列(NOT NULL DEFAULT 0)+索引+回填根组织单租户。</summary>
    private static void MigrateV63(STOTOPDbContext ctx)
    {
        if (!SeederHelper.IsSqlServer(ctx)) return;

        SeederHelper.ExecuteRawSql(ctx, @"
        IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.COLUMNS
            WHERE TABLE_NAME = N'CF编号序号' AND COLUMN_NAME = N'F租户ID')
        ALTER TABLE [CF编号序号] ADD [F租户ID] bigint NOT NULL CONSTRAINT [DF_CF编号序号_F租户ID] DEFAULT 0;");

        SeederHelper.ExecuteRawSql(ctx, @"
        IF NOT EXISTS (SELECT * FROM sys.indexes
            WHERE name = N'IX_CF编号序号_租户ID' AND object_id = OBJECT_ID(N'CF编号序号'))
        CREATE INDEX [IX_CF编号序号_租户ID] ON [CF编号序号] ([F租户ID]);");

        SeederHelper.ExecuteRawSql(ctx, @"
        DECLARE @tenant bigint = (SELECT TOP 1 [FID] FROM [SYS组织架构] WHERE [F父ID] = 0 ORDER BY [FID]);
        IF @tenant IS NOT NULL
            UPDATE [CF编号序号] SET [F租户ID] = @tenant WHERE [F租户ID] = 0;");
    }
