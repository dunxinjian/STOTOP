...
public static class ContractSeeder
{
    ...
    /// <summary>阶段0 多租户隔离：需加 F租户ID 的 7 张租户表（全覆盖，见 design/24-tenant-migration-playbook.md）。</summary>
    private static readonly string[] Phase0TenantTables =
    {
        "CON合同", "CON合同条款", "CON合同提醒", "CON电子签记录",
        "CON合同模板", "CON合同方", "CON合同类型",
    };
    ...
    public static void Migrate(STOTOPDbContext ctx)
    {
        ...
        {
            new(1, "阶段0多租户: Contract 7张租户表加 F租户ID 隔离键列(NOT NULL DEFAULT 0,不启用过滤器)+租户索引 (2026-07-01)", MigrateV1),
            new(2, "阶段0多租户: Contract 存量行 F租户ID 回填到根组织单租户(=根组织id) (2026-07-01)", MigrateV2),
        });
    }
