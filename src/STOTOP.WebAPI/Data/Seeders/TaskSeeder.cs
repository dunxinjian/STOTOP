...
{
    private const string Module = "Task";

    /// <summary>阶段0 多租户隔离：需加 F租户ID 的 25 张租户表（全覆盖，见 design/24-tenant-migration-playbook.md）。</summary>
    private static readonly string[] Phase0TenantTables =
    {
        "TM钉钉消息推送", "TM目标", "TM钉钉待办", "TM活动日志", "TM关键成果",
        "TM附件", "TM评论表情", "TM知识评论", "TM知识库", "TM知识互动",
        "TM评价维度配置", "TM站内通知", "TM绩效考核周期", "TM绩效考核记录", "TM绩效维度评分",
        "TM进度上报", "TM项目", "TM项目成员", "TM复盘记录", "TM任务评论",
        "TM任务", "TM任务参与者", "TM任务提醒", "TM标签", "TM任务依赖",
    };
    ...
    public static void Migrate(STOTOPDbContext ctx)
    {
        ...
        {
            new(1, "阶段0多租户: Task 25张租户表加 F租户ID 隔离键列(NOT NULL DEFAULT 0,不启用过滤器)+租户索引 (2026-07-01)", MigrateV1),
            new(2, "阶段0多租户: Task 存量行 F租户ID 回填到根组织单租户(=根组织id) (2026-07-01)", MigrateV2),
        });
    }
